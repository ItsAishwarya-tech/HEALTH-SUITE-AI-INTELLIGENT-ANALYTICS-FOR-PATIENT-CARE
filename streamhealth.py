...
#         st.success(f"🕒 Predicted Length of Stay: **{prediction[0]:.2f} days**")
    ...
    st.title("Length of Stay Prediction (Regression)")
    st.write("Forecast patient hospitalization duration for resource planning.")
...
# ========== IMAGING DIAGNOSTICS ==========
    ...
    if uploaded_file:
        ...
        demo_diseases = [
            ...
            "COVID-19 Signs Detected"
        ]
        prediction = random.choice(demo_diseases)
        st.success(f"Prediction: {prediction} (Demo)")
